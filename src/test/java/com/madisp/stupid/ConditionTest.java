...
public class ConditionTest extends BaseExpressionTest {
    ...
    public void testIf() throws StupidRuntimeException {
        ...
        assertEquals(1, eval("if (true) { 1 }"));
        assertEquals(1, eval("if (true) { a=1;a; }"));
        assertEquals(1, eval("if (1==1) { a=1;a; }"));
        assertEquals(2, eval("x = if (1==1) { a=1;a=a+1;a; } else {b=24;b}; x;"));
        assertEquals(2, eval("x = if (1==1) { a=1;a=a+1;a } else {b=23;b}; x;"));
        ctx.popExecContext();
    }
    ...
    public void testIfElse() throws StupidRuntimeException {
        ...
        assertEquals(2, eval("if (false) {22}else{ a=2;a; }"));
        assertEquals(2, eval("if (false) {22}else{ a=2;a }"));
        assertEquals(2, eval("x = if (1==3) { a=1;a; } else {b=2;b}"));
        assertEquals(2, eval("x = if (1==3) { a=1;a; } else {b=2;b}; x"));
        ctx.popExecContext();
    }
